...
console.log("🚀 Applying Next.js 15 Params Fix...");
  ...
  return (
    <div ref={setNodeRef} style={style} className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-4 relative group hover:border-blue-500 transition cursor-default">
      ...
      <div className="pl-10">
        ...
        {type === 'checkout' && <div className="bg-white text-black p-6 rounded-lg text-center max-w-sm mx-auto shadow-xl"><h3 className="font-bold text-xl mb-2">Premium Masterclass</h3><p className="mb-4 text-gray-600">Lifetime Access • KES 5,000</p><div className="w-full"><PaystackButton {...paystackConfig} className="bg-green-600 hover:bg-green-700 text-white w-full py-3 rounded font-bold transition" /></div><p className="text-xs text-gray-400 mt-2">Secured by Paystack</p></div>}
...
export default function FunnelEditor({ params }: { params: Promise<{ id: string }> }) {
  // 1. UNWRAP PARAMS (The Next.js 15 Fix)
  const { id } = use(params);
...
fs.writeFileSync('app/dashboard/funnels/[id]/page.tsx', content);
console.log("✅ Fix Applied! Restart your server.");