...
export const ProductCard = ({ id, name, price, isPublished }: any) => {
  ...
  const onDuplicate = async () => {
    try {
      ...
      router.refresh();
    } catch { toast.error("Error duplicating"); } finally { setLoading(false); }
  };

  return (
    <div className="bg-[#0B0F1A] border border-slate-800 rounded-xl p-6 hover:border-indigo-500/50 transition">
      <div className="flex justify-between items-start mb-4">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="h-8 w-8 p-0 text-slate-400"><MoreVertical className="h-4 w-4" /></Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="bg-[#161B22] border-slate-700 text-slate-200">
            <Link href={"/dashboard/funnels/" + id}><DropdownMenuItem className="cursor-pointer"><Edit className="h-4 w-4 mr-2" />Edit Details</DropdownMenuItem></Link>
      ...
      <div className="flex items-center gap-3 mt-2">
        <span className="text-[10px] px-2 py-0.5 rounded-full bg-green-500/10 text-green-400 border border-green-500/20">{isPublished ? "LIVE" : "DRAFT"}</span>
