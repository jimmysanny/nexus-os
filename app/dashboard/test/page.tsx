...
export default async function TestEmailPage({ searchParams }: { searchParams: { sent?: string } }) {
  const user = await currentUser();
  const email = user?.emailAddresses[0]?.emailAddress || "jimmysanny01@gmail.com";
  
  // Await searchParams to satisfy Next.js 15 rules
  ...
  async function triggerTestEmail() {
    ...
    // Simulate a fake transaction of KES 1,000
