...
import { CheckCircle, ShieldCheck, Zap, Globe, Smartphone, Lock, ArrowRight, Download, Eye, FileText } from "lucide-react";
...
export default async function FunnelPage({ params }: FunnelPageProps) {
  ...
  const { userId } = await auth(); // Identify who is looking
  ...
  // CHECK: Is the viewer the Creator?
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="pt-8 border-t border-slate-800">
              ...
              {isOwner ? (
                // --- OWNER VIEW (ACCESS FILE) ---
                <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-6">
                  <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-2">
                    <ShieldCheck className="h-5 w-5 text-green-500" /> 
                    Your Product File
                  </h3>
                  <p className="text-slate-400 text-sm mb-6">Since you own this product, you can access the content directly.</p>
                  ...
                  {product.fileUrl ? (
                     ...
                     <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 rounded-lg text-center">
                       No file uploaded yet. Go to Dashboard to add one.
                     </div>
                  )}
                </div>
              ) : (
                // --- CUSTOMER VIEW (PAY WALL) ---
                <>
          ...
          <div className="relative aspect-video rounded-2xl overflow-hidden border border-slate-800 bg-[#0B0F1A] shadow-2xl shadow-indigo-500/10">
            ...
            ) : product.fileUrl && isVideo(product.fileUrl) ? (
               <div className="flex items-center justify-center h-full bg-slate-900">
                 <div className="text-center">
                   <div className="h-16 w-16 bg-slate-800 rounded-full flex items-center justify-center border border-slate-700 mx-auto mb-4">
                     <Eye className="h-8 w-8 text-indigo-500" />
                   </div>
                   <p className="text-slate-400 font-medium">Video Preview Available After Purchase</p>
                 </div>
               </div>
            ) : (
